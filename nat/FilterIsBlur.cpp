...
void HwtFirstRound(const uint8_t* const data, int height, int width,
  ...
  for (int i = 0; i < num_rows; ++i) {
    int32_t* a_tmp = ptr_a;
    const uint8_t* data_tmp = ptr_data;
    for (int j = 0; j < half_num_columns; ++j) {
...
int ComputeEdgePointWeight(int* matrix, int width, int height,
  ...
  int v = sqrt((float)(v_top_right + v_bot_left + v_bot_right));
  return v;
}
...
int ComputeLocalMaximum(int* matrix, int width, int height,
  ...
  *l = left;
  ...
  int v_bot_right=square(matrix[(r + scaled_height) * width + c + scaled_width]);
  int v = sqrt((float)(v_top_right + v_bot_left + v_bot_right));
  return v;
}
