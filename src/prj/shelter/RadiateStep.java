...
public abstract class RadiateStep extends Stepper {
	...
	protected GridPane gen_layout() {
		...
		lay.addColumn(5, new Label("強度"), box_stng);
		lay.add(chk_meas, 6, 0);
		lay.add(chk_mark, 6, 1);
		lay.add(txt_meas, 2, 2, 4, 1);
		return lay;
	};
	...
	protected void event_add_mark() {
		if(chk_meas.isSelected()==false || chk_mark.isSelected()==false) { 
			return; 
		}		
		abacus.addMark(
			box_stng.getValue(), 
	...
	protected final Runnable op_make_radiation = ()->{
		if(hustio.isRadiant()==true) {
			show_info("照射中","busy!!");
			Misc.logv("[HOSTIO] radiation again!!!");
			hold_step();
		}else {
			final DevHustIO.Strength stng = box_stng.getSelectionModel().getSelectedItem();
			show_info(
				stng.toString(),
				"00:00:00",
				box_left.getText()
			);
			hustio.asyncRadiation(stng, Misc.text2tick(box_left.getText()));
			next_step();		
		}
	};
	...
	public static class Reset extends RadiateStep {
		...
		final JFXCheckBox chk3 = new JFXCheckBox("清除模型資料");
