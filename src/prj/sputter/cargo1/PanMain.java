...
public class PanMain extends PanBase {
	...
	public static DevAdam4055 adam1 = new DevAdam4055(1);//shutter 1~3, MFC valve 1~3
	public static DevAdam4068 adam2 = new DevAdam4068(2);//relay for wire-hub!!!!
	...
	public static final ReadOnlyFloatProperty mfc1_pv = LayTool.transform(
		adam3.ain[0].val, src->{
		return (src*100f)/5f; //0~5V --> 0~100sccm
	});
	...
	private Node genValvePan() {
		...
		act_on_off.setOnAction(e->{
			...
			lay0.addColumn(0, chk[0], chk[2], chk[4]);
			lay0.addColumn(1, chk[1], chk[3], chk[5]);
		...
		CheckBox chk10 = PanBase.genIndicator("擋板-0", adam1.DI[0]);
	...
	private Node gen_MFC_Pan() {
		...
		//SV
		txt[1][1].setOnMouseClicked(bind_MFC_SV(txt[1][2],adam4.aout[0].val,100));
		txt[2][1].setOnMouseClicked(bind_MFC_SV(txt[2][2],adam4.aout[1].val,100));
		txt[3][1].setOnMouseClicked(bind_MFC_SV(txt[3][2],adam4.aout[2].val,100));
	...
	private EventHandler<? super MouseEvent> bind_MFC_SV(
		...
		final ReadOnlyFloatProperty sv = LayTool.transform(prop, src->{
		...
		final EventHandler<? super MouseEvent> event = (e)->{
			...
			try {
				//0~100sccm --> 0~5V 
				final float sccm = Float.parseFloat(opt.get());
				final float volt = (sccm*5f)/MAX_SCCM;
				prop.set(volt);
